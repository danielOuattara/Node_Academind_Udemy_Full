...
exports.postResetPassword = (req, res, next) => {
  ...
  User.findOne({email: req.body.email})
    .then( user => {  // verify user exist in DB!
      if(!user) {  // user not found in DB
        req.flash('ErrorLogin','Invalid email OR password')
        return res.redirect('/login');
      }

      crypto.randomBytes(32, (err, buffer) => {  // generate a Token
        if(err) {
          req.flash('ErrorLogin','Invalid email OR password')
          console.log(err)
      ...
      .then( () => {
        res.redirect('/login')
        transporter.sendMail({  // send email to user for resetting password
          to: req.body.email,
          from: 'from_me@gmail.com',
          subject: 'Password reset !',
          html:` 
          <p>Your request a password rest</p>
          <p>Click <a href="http://localhost:3000/resetpassword/${token}">Reset Password</a> to reset your password</p>
          `
        })
      })
      .catch(err => {
...
exports.postRenewPassword = (req, res, next) => {
  User.findOne({resetPasswordToken: req.params.token, resetPasswordTokenExpiration: {$gt: Date.now()}})
  .then()
  .catch(err => console.log(err))
